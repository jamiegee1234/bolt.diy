...
import { getDebugData, enhancedLogger, type ErrorReport, type PerformanceMetric } from '~/lib/enhanced-logging';
import { MemoryOptimizer } from '~/components/ui/MemoryOptimizer';
...
export default function DebugTab() {
  ...
  const [enhancedDebugData, setEnhancedDebugData] = useState<{
    errors: ErrorReport[];
  ...
  const loadEnhancedDebugData = async () => {
    try {
      setLoading((prev) => ({ ...prev, enhanced: true }));

      const debugData = getDebugData();
      setEnhancedDebugData(debugData);

      const errorCount = debugData.errors.filter(e => e.level === 'error').length;
      const warningCount = debugData.errors.filter(e => e.level === 'warning').length;
      const slowMetrics = debugData.performance.filter(m => m.duration > 100).length;

      toast.success(`Loaded: ${errorCount} errors, ${warningCount} warnings, ${slowMetrics} slow operations`);
    } catch (error) {
      toast.error('Failed to load enhanced debug data');
      console.error('Failed to load enhanced debug data:', error);
    } finally {
