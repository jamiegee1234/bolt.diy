...
import { registerCleanupListener, isMemoryUnderPressure } from '~/lib/memory-optimization';
...
export class FilesStore {
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    // Register for memory optimization cleanup
    if (typeof window !== 'undefined') {
      registerCleanupListener('file', (detail) => {
        this.#optimizeMemoryUsage(detail.maxSize || 50);
      });
    }
  ...
  /**
   * Optimize memory usage by removing least recently used files from memory
   */
  #optimizeMemoryUsage(maxSize: number) {
    try {
      const currentFiles = this.files.get();
      const fileEntries = Object.entries(currentFiles);
      
      // If we're under the limit, no need to optimize
      if (fileEntries.length <= maxSize) {
        return;
      }

      logger.debug(`Optimizing memory: ${fileEntries.length} files, target: ${maxSize}`);

      // Sort files by type and size - keep smaller, more recently accessed files
      const filesToRemove = fileEntries
        .filter(([, dirent]) => dirent?.type === 'file')
        .sort((a, b) => {
          const aFile = a[1] as File;
          const bFile = b[1] as File;
          
          // Prioritize removing large files
          return bFile.content.length - aFile.content.length;
        })
        .slice(maxSize)
        .map(([path]) => path);

      // Remove excess files from memory (but keep the file references)
      if (filesToRemove.length > 0) {
        const optimizedFiles = { ...currentFiles };
        
        filesToRemove.forEach(filePath => {
          if (optimizedFiles[filePath]?.type === 'file') {
            // Keep the file entry but clear the content to save memory
            optimizedFiles[filePath] = {
              type: 'file',
              content: '', // Clear content to save memory
              isBinary: (optimizedFiles[filePath] as File).isBinary,
              isLocked: (optimizedFiles[filePath] as File).isLocked,
              lockedByFolder: (optimizedFiles[filePath] as File).lockedByFolder,
            };
          }
        });

        this.files.set(optimizedFiles);
        logger.debug(`Memory optimization: cleared content for ${filesToRemove.length} files`);
      }
    } catch (error) {
