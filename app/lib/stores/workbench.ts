...
export class WorkbenchStore {
  ...
  // Configuration for action sampling interval (configurable alternative to magic number)
  #actionSampleInterval = parseInt(globalThis.process?.env?.BOLT_ACTION_SAMPLE_INTERVAL || '100');
  
  // Track running actions for abort functionality
  #runningActions = new Set<string>();
  ...
  abortAllActions() {
    console.log('Aborting all running actions...');
    
    try {
      // Stop all running actions by clearing the execution queue
      this.#globalExecutionQueue = Promise.resolve();
      
      // Clear running actions tracker
      this.#runningActions.clear();
      
      // Abort any running artifacts
      const artifacts = this.artifacts.get();
      Object.values(artifacts).forEach(artifact => {
        if (artifact.runner) {
          try {
            // If the runner has an abort method, call it
            if (typeof artifact.runner.abort === 'function') {
              artifact.runner.abort();
            }
          } catch (error) {
            console.warn(`Failed to abort artifact ${artifact.id}:`, error);
          }
        }
      });
      
      // Reset terminal state to recover from potentially stuck processes
      this.#terminalStore.reset();
      
      console.log('All actions aborted successfully');
    } catch (error) {
      console.error('Error during abort operation:', error);
      
      // Force recovery by resetting critical state
      this.#runningActions.clear();
      this.#globalExecutionQueue = Promise.resolve();
    }
  }
  ...
  async _runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    if (data.action.type === 'file') {
      ...
      // Remove from tracking when completed or failed
  ...
  }, this.#actionSampleInterval); // Configurable sampling interval (set via BOLT_ACTION_SAMPLE_INTERVAL env var, defaults to 100ms)
