...
export function useGit() {
  ...
  const gitClone = useCallback(
    async (url: string, retryCount = 0) => {
      ...
      // Try to get saved GitHub credentials first
      let auth = lookupSavedPassword(url);
      
      // If no saved credentials, check for GitHub token from connection
      ...
      try {
        ...
        await git.clone({
          ...
          onAuth: (url) => {
            ...
            // Try GitHub token for github.com URLs
            ...
            const message = url.includes('github.com') 
              ? 'This is a private GitHub repository. You can:\n\n1. Connect your GitHub account in Settings > Connections\n2. Or enter your credentials manually\n\nWould you like to enter credentials now?'
              : 'This repository requires authentication. Would you like to enter your credentials?';

            if (confirm(message)) {
              const username = prompt('Enter username or GitHub token:') || '';
              const password = url.includes('github.com') && username.startsWith('ghp_') 
                ? 'x-oauth-basic'  // If it looks like a GitHub token, use x-oauth-basic as password
          ...
          onAuthFailure: (url, _auth) => {
            ...
            let message = `Authentication failed for ${url.split('/')[2]}.`;
            
            if (url.includes('github.com')) {
              message += '\n\nTo access private repositories:\n1. Go to Settings > Connections\n2. Connect your GitHub account\n3. Or use a personal access token with repo permissions';
            } else {
        ...
        if (errorMessage.includes('Authentication failed')) {
          ...
          const message = isGitHub 
            ? `Unauthorized access to repository. This repository may be private.\n\nTo access private repositories:\n1. Go to Settings > Connections\n2. Connect your GitHub account\n3. Ensure your token has 'repo' permissions`
